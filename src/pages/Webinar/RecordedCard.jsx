    ...
    >
        ...
        <div className="card-cover">
          ...
          <div className="card-overlay">
            ...
            <div className="card-overlay-meta">
              {item.dateTime ? new Date(item.dateTime).toLocaleString('en-US', { month: 'short', day: 'numeric', year: 'numeric', hour: 'numeric', minute: 'numeric', hour12: true }) : 'Recorded'}
            </div>
          </div>
        </div>
      )}
      <div style={{ padding: '16px' }}>
        ...
        <div style={{ marginTop: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <button className="btn" onClick={() => navigate(`/webinar/${item.id}`, { state: { item } })} style={{ padding: '8px 16px', fontSize: '13px' }}>Watch</button>
        </div>
      </div>
    </motion.div>
  )
